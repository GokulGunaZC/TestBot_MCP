...
`;
  ...
  async executePlaywright({ project, lastFailed = false, grep, grepInvert, outputDir, configPath: configPathOverride } = {}) {
    ...
    for (let attempt = 0; attempt <= crashRetries; attempt += 1) {
      ...
      commandResult = await this.runPlaywrightCommand(args, timeoutMs, {
      ...
      const producedNoResults = commandResult.code !== 0 && Number(testResults.total || 0) === 0;
      if (!producedNoResults || attempt >= crashRetries) {
        break;
      }

      Logger.warn('PlaywrightIntegration', 'Playwright produced no parseable results; retrying once as infrastructure crash', {
        attempt: attempt + 1,
