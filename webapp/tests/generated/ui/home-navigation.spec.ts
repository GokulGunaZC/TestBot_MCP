...
async function mockAuthenticatedAppShell(page: Page) {
  ...
  await page.route('**/api/test-runs', async (route: Route) => {
    await route.fulfill({
      status: 200,
  ...
  await page.route('**/api/test-lists', async (route: Route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
...
test.describe('Home dashboard navigation [CAT:ui_flow]', () => {
  ...
  test('home loads and shows primary dashboard actions [CAT:ui_flow]', async ({ page }) => {
    ...
    if (new URL(page.url()).pathname === '/login') {
      ...
      await expect(page.getByText(/log\s*in|sign\s*in/i).first()).toBeVisible();
