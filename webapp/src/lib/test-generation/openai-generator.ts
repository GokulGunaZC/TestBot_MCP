...
- Avoid exact concatenated accessible names for cards containing multiple text nodes. Locate by the stable card title text first, then scope metadata assertions inside the nearest card/container when possible.
...
- Calendar events are month-specific. If a test clicks Next Month or Previous Month, assert the heading/navigation state first; do not assert May event chips such as Standup/Planning/Review remain visible in a different month unless context proves recurring events.
